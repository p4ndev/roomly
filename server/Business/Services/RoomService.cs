...
{
    public async Task<RoomEntity?> CreateAsync(RoomDto model, CancellationToken token = default)
    {
    ...
    public async Task<IList<RoomEntity>> ListAsync(CancellationToken token = default)
        => await _relationalContext.Rooms.AsNoTracking().OrderBy(r => r.Name).ToListAsync(token);
}